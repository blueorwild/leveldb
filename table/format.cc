...
void Footer::EncodeTo(std::string* dst) const {
  PutVarint64(dst, index_block_offset_);
  PutVarint32(dst, index_block_size_);
  PutVarint32(dst, index_count_);
  PutVarint32(dst, kv_num_);
  PutVarint32(dst, filter_name_.size());
  dst->append(filter_name_);
  dst->resize(Footer::kFooterSize - 8);  // Padding
  PutFixed32(dst, static_cast<uint32_t>(kTableMagicNumber & 0xffffffffu));
  PutFixed32(dst, static_cast<uint32_t>(kTableMagicNumber >> 32));
}

Status Footer::DecodeFrom(Slice* input) {
  const char* magic_ptr = input->data() + Footer::kFooterSize - 8;
